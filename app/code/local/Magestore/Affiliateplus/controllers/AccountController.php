...
class Magestore_Affiliateplus_AccountController extends Mage_Core_Controller_Front_Action {
    ...
    public function editPostAction() {
        ...
        $bank_account_data = array(
            ...
            "swift_code" => $data["bankswiftcode"]
            
        );
        $bankAccount = Mage::getModel('affiliatepluspayment/bankaccount')
                    ->load($session->getAccount()->getId(),'account_id');
        $bankAccount->addData($bank_account_data);  
        //end tab      
        try {
            /*
    ...
    public function createPostAction() {
        ...
        if ($session->isRegistered()) {
            ...
            //tab
            $image = '';
            if(isset($_FILES['bankstatement']['name']) && $_FILES['bankstatement']['name'] != '') {
                $image = $_FILES['bankstatement']['name'];
                try {
                    $uploader = new Varien_File_Uploader('bankstatement');
                    $uploader->setAllowedExtensions(array('jpg','jpeg','gif','png')); 
                    $uploader->setAllowRenameFiles(false);
                    $uploader->setFilesDispersion(false);        
                    $path = Mage::getBaseDir('media') . DS ;
                    $path = $path ."affiliateplus/bankstate";
                    $uploader->save($path, $_FILES['bankstatement']['name']);
                } catch (Exception $e) {
        ...
        try {
            ...
            // Tab
            if (!Mage::helper('affiliateplus/config')->getSharingConfig('required_address')) {
                $telephone = Mage::helper('affiliateplus')->refineUrlKey($this->getRequest()->getPost('telephone'));
                $successMessage = Mage::helper('affiliateplus/account')->createAffiliateAccount($address, $this->getRequest()->getPost('paypal_email'), $customer, $this->getRequest()->getPost('notification'), $this->getRequest()->getPost('referring_website'), $successMessage, $referredBy, $coreSession, $keyShop, $telephone);
            } else {
            ...
            $accountId = Mage::getModel('affiliateplus/account')->load($customerEmail,'email')->getId();
            $bankAccount = $bankAccount->setAccountId($accountId)
                                        ->setId(null)
                                        ->save();
